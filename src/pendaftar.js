...
export async function scrapePendaftar({ url, sekolahID, type }) {
  if (!url) {
  ...
  const title = `${startTimer
    ...
    .slice(
      0,
      10
    )} ${startTimer.getHours()} ${startTimer.getMinutes()} ${startTimer.getSeconds()}`;
  ...
  try {
    ...
    console.log(
      `> Hasil Seleksi(SEMENTARA) ${
        hasil + 1 <= 28 ? "Lulus" : "Tidak"
      } dengan nomor urut ${hasil + 1}`
    );
    console.log("> Done!\n");
  } catch (err) {
    throw new Error(err);
